...
fact chainRule{
  ...
  all p: Process | one s: State | p.state = s
  ...
  // Ensure some process chains are allowed
  some p: Process | some p.nextProcess
  //no loop in process chain
  //no p: Process | p in (p.^nextProcess)
 // no p1, p2:Process | p1!=p2 => (p1.nextProcess = p2) && (p2.nextProcess = p1)
}
...
pred readyToRunning[s, s": Scheduler] {
  ...
  let selectedProcess = s.ready {
	 ...
	 s".running = selectedProcess
    	 s".ready = s.ready - selectedProcess
	 s".waiting = s.waiting 
  }
...
pred waitingToReady[s,s":Scheduler] {
   let waitingProcess = s.waiting {
	...
	s".waiting = s.waiting - waitingProcess 
	s".running = s.running
   }
}
...
pred runningToWaiting[s,s":Scheduler] {
   let runningProcess = s.running {
	...
	s".waiting = s.waiting + runningProcess
	s".running = none  
   }
}
